...
let
  ...
  trackedPackageNames = lib.unique (
    map lib.getName (config.environment.systemPackages ++ hmPackages)
    ++ cfg.extraTrackedPackages
  );
  ...
  config = lib.mkIf cfg.enable {
    systemd.services.nixos-flake-update = {
      ...
      serviceConfig = {
        ...
        StandardOutput = "append:/var/log/nixos-flake-update/update.log";
        StandardError = "append:/var/log/nixos-flake-update/update.log";
        Nice = 19;
