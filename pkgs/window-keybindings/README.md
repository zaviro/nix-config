...
该 helper 已作为 `nix-config` monorepo 的根 Flake package 暴露。在
`nix-config` 根目录运行：

```bash
nix build .#window-keybindings
nix run .#window-keybindings -- inspect
```

运行时依赖由根 Flake 的 `nixpkgs` 提供：`niri`、`jq`、`flock`。
