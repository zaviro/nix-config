...
作为 `nix-config/pkgs/window-keybindings` 子项目嵌入时：

- `profiles/v0_7.nix` 只声明参考键位与 contextual/singleton policy，不包含主机 app_id；
- `examples/` 只展示 adapter 调用方式，不参与任何主机部署；
- 上游 `flake.nix` 保留独立开发入口，`default.nix` 是根 Flake 的 `callPackage` 适配；
- 实际部署模块负责选择 profile，并补齐真实 app_id、启动命令与 compositor 渲染。

因此子项目可以独立迭代规范、helper、参考 profile 与示例，同时不会自己写入任何主机的
Niri 配置。
